...
- **1-3.** 分析请求体特征
  - 从 `${alert:httpRequest}` 识别是否存在加密特征（冰蝎、哥斯拉使用AES等加密）
  - 识别是否存在base64编码的命令执行特征
  - 识别是否存在常见的webshell参数名: `pass`, `password`, `cmd`, `command`, `shell`, `c`, `e`, `exec`
...
- **3-1.** 分析URL特征
  ...
  - 检查是否为随机命名的文件: `/uploads/ab12cd34.php`, `/images/x9k2m.php`
...
- **2-2.** 分析URL路径
  ...
  - 判断文件名是否符合业务命名规范（如 `api.php`, `upload.php`, `login.php`）
...
#### 4. 历史流量分析
- **4-1.** 分析HTTP请求历史
  - 通过 `${soc-getHttpLog}` 获取该目的IP近7天的http请求日志
  - 判断该URL是否长期存在正常的业务访问
